...
pub fn tap_row(w: f32, h: f32, bg: u32, route: &str) -> Option<Node> {
    Some(row(w, h, bg)?.on_event(crate::arkui::event::click(), intern(route)))
}

pub fn tap_row_fit(w: f32, bg: u32, route: &str) -> Option<Node> {
    Some(row_fit(w, bg)?.on_event(crate::arkui::event::click(), intern(route)))
}
...
pub fn act_row(w: f32, h: f32, bg: u32, action: &str) -> Option<Node> {
    Some(row(w, h, bg)?.on_event(crate::arkui::event::click(), intern_action(action)))
}

pub fn act_row_fit(w: f32, bg: u32, action: &str) -> Option<Node> {
    Some(row_fit(w, bg)?.on_event(crate::arkui::event::click(), intern_action(action)))
}
