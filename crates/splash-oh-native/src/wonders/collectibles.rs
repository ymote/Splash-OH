...
pub fn load() {
    for path in STORES {
        ...
        crate::log(&format!("wonders/collectibles: loaded from {path}"));
        return;
    }
    crate::log("wonders/collectibles: nothing stored yet");
}
...
fn save() {
    ...
    for path in STORES {
        ...
        match std::fs::write(path, &bits) {
            Ok(()) => {
                crate::log(&format!("wonders/collectibles: saved to {path}"));
                return;
            }
            Err(e) => crate::log(&format!("wonders/collectibles: {path}: {e}")),
        }
    }
}
...
pub fn badge(i: usize, size: f32) -> Option<Node> {
    ...
    Some(
        stack(size, size, 0x33F8ECE5)?
            ...
            .on_event(crate::arkui::event::click(), COLLECT_BASE + i as i32),
    )
}
