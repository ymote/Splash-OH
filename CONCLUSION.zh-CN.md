...
> **一个被撤回的异常值。** TikTok 一开始测出来是 +50%，文档里当成"无法解释"报了
> 出去。那是测试装置的 bug，不是 ArkTS 的性质。计时那一组用的 `build_route` 把
> TikTok 的 `feed` 路由映射到 `build_feed()`（五个 reel 全建）；而**内存**那一组
> 用的 `set_route` 里根本没有 `feed` 这个分支，直接落到了单个 reel。于是 Rust 只
> 拿着一个 reel，ArkTS 拿着五个，那个差距是工作量的差，不是开销的差。修完之后
> TikTok 是 +17%，异常值消失了。
>
> 值得说一下它是怎么被发现的：不是 benchmark 发现的，它给出的数字干干净净；是因为
> 追问"为什么这一个 app 跟另外三个不一样"。**没解释清楚的结果，在被证明之前都该
> 当成缺陷。**
